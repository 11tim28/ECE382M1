...
int GetJ(int *x)             { return((x[J5] << 5) + (x[J4] << 4) +
				      (x[J3] << 3) + (x[J2] << 2) +
				      (x[J1] << 1) + x[J0]); }
int GetLD_MAR(int *x)        { return((x[LD_MAR1] << 1) + x[LD_MAR0]); }
...
int GetLD_rState(int *x)    { return(x[LD_rState]);}
int GetRT(int *x)           { return(x[RT]);}
int GetMDR_setM(int *x)     { return(x[MDR_setM]);}
int GetMDR_setR(int *x)     { return(x[MDR_setR]);}
int GetLD_MDR_TMP(int *x)   { return(x[LD_MDR_TMP]);}
...
int rState; /* Return state after address translation */
int MDR_TMP; /* tempory MDR for interrupt */
...
void rdump(FILE * dumpsim_file) {                               
    ...
    printf("VA           : 0x%0.4x\n", CURRENT_LATCHES.VA);
    printf("rState       : 0x%0.4x\n", CURRENT_LATCHES.rState);
    printf("MDR_TMP      : 0x%0.4x\n", CURRENT_LATCHES.MDR_TMP);
   ...
   */
   ...
   /* !!! NOT SURE 1/0 !!!*/
    if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION)) {
        if(GetRT(CURRENT_LATCHES.MICROINSTRUCTION)){
            // return from address translation
            NEXT_LATCHES.STATE_NUMBER = CURRENT_LATCHES.rState;
        }
        else{
            // opcode mode
...
//             else if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 2) NEXT_LATCHES.MAR = CURRENT_LATCHES.PTBR + ((CURRENT_LATCHES.VA & 0xFE00) >> 9);
...
//    }
   ...
   if(GetLD_MAR(CURRENT_LATCHES.MICROINSTRUCTION) == 1) NEXT_LATCHES.MAR = BUS;
    ...
    /* MDR_TMP */
   if(GetLD_MDR_TMP(CURRENT_LATCHES.MICROINSTRUCTION)) NEXT_LATCHES.MDR_TMP = CURRENT_LATCHES.PSR;
   ...
   if(GetLD_MDR(CURRENT_LATCHES.MICROINSTRUCTION)){
        ...
        if(CURRENT_LATCHES.STATE_NUMBER == 0x003B){
            // state 59
            NEXT_LATCHES.MDR = CURRENT_LATCHES.MDR_TMP;
        }
   }
   ...
   if(GetLD_rState(CURRENT_LATCHES.MICROINSTRUCTION) && NEXT_LATCHES.STATE_NUMBER != 0x0029){
    // 0x0029 == state 41 (the begin of interrupt)
    NEXT_LATCHES.rState = GetJ(CURRENT_LATCHES.MICROINSTRUCTION);
    if(GetR_W(CURRENT_LATCHES.MICROINSTRUCTION)){
        // write translation
        NEXT_LATCHES.STATE_NUMBER = 0x003A; // state 58
        memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER], sizeof(int)*CONTROL_STORE_BITS);
    }
    else{
        // read translation
        NEXT_LATCHES.STATE_NUMBER = 0x0039; // state 57
        memcpy(NEXT_LATCHES.MICROINSTRUCTION, CONTROL_STORE[NEXT_LATCHES.STATE_NUMBER], sizeof(int)*CONTROL_STORE_BITS);
    }
   }
