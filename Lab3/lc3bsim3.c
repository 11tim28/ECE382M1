...
void eval_micro_sequencer() {
   ...
   */
    ...
    if(GetIRD(CURRENT_LATCHES.MICROINSTRUCTION)) {
        ...
        NEXT_LATCHES.STATE_NUMBER |= ((cond1 & 0b1) & CURRENT_LATCHES.BEN & ~(cond0 & 0b1)) << 2;
        NEXT_LATCHES.STATE_NUMBER |= ((cond0 & 0b1) & ~(cond1 & 0b1) & CURRENT_LATCHES.READY) << 1;
        NEXT_LATCHES.STATE_NUMBER |= ((cond1 & 0b1) & (cond0 & 0b1) & ((CURRENT_LATCHES.IR >> 11) & 0x0001));
    }
...
void drive_bus() {
   ...
   */       
   ...
   /* ALU */
    int sr1, sr2;
    if(GetSR1MUX(CURRENT_LATCHES.MICROINSTRUCTION)) sr1 = (CURRENT_LATCHES.IR >> 6) & 0x0007;
    ...
    if(BUS_MUX == 1){ // MARMUX
        ...
        if(((CURRENT_LATCHES.IR >> 4) & 0x0003) == 0x0000) BUS = Low16bits(SR1_OUT << amount4);
